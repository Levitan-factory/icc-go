...
export function parseCellDsl(
  ...
  const uses: ParsedDsl["outputs"]["uses"] = [];
  ...
  for (const [index, rawLine] of headerLines.entries()) {
    ...
    if (line.includes("/*") || line.includes("*/")) {
      diagnostics.push({
        level: "error",
        line: lineNumber,
        message: "Sender notes are body-only. Move `/* ... */` below the control header.",
      });
    }
    ...
    if (line.startsWith(">")) {
      if (routing) {
        diagnostics.push({
          level: "error",
          line: lineNumber,
          message: "Only one routing line is allowed per cell. Use a second cell and %from cN for a second model step.",
        });
        continue;
      }
  ...
  const parsedBody = parsePromptBodySegments(promptBody, headerLines.length);
  diagnostics.push(...parsedBody.diagnostics);

  const references = parseReferences(`${controlHeader}\n${parsedBody.sourceText}`, diagnostics, options);
  ...
  return {
    ...
    flow,
    outputs,
    bodySegments: parsedBody.segments,
    senderNotes: parsedBody.senderNotes,
    references,
    dependencies,
...
function parsePromptBodySegments(promptBody: string, headerLineCount: number): ParsedBodySegments {
  const normalized = promptBody.replace(/\r\n/g, "\n");
  const lines = normalized.split("\n");
  const segments: BodySegment[] = [];
  const diagnostics: Diagnostic[] = [];
  const senderNotes: string[] = [];
  let inNote = false;
  let noteStartLine: number | undefined;
  let inFence = false;
  let fenceMarker: "`" | "~" | undefined;

  function append(type: BodySegment["type"], text: string, line: number) {
    if (!text) return;
  ...
  function pushDiagnostic(line: number, message: string) {
    diagnostics.push({ level: "error", line: headerLineCount + line, message });
  }

  for (const [index, rawLine] of lines.entries()) {
    const bodyLine = index + 1;
    const lineText = index < lines.length - 1 ? `${rawLine}\n` : rawLine;
    const fence = rawLine.trimStart().match(/^(```+|~~~+)/);

    if (!inNote && fence) {
      append("source_text", lineText, bodyLine);
    ...
    while (cursor < lineText.length) {
      if (!inNote && lineText.startsWith("\\/*", cursor)) {
        append("source_text", "/*", bodyLine);
        cursor += 3;
        continue;
      }

      if (inNote && lineText.startsWith("*\\/", cursor)) {
        append("sender_note", "*/", bodyLine);
        cursor += 3;
        continue;
      }
      ...
      if (!inNote && lineText.startsWith("/*", cursor)) {
        inNote = true;
        noteStartLine = bodyLine;
        cursor += 2;
        continue;
      }

      if (inNote && lineText.startsWith("/*", cursor)) {
        pushDiagnostic(bodyLine, "Nested sender notes are not supported.");
        append("sender_note", "/*", bodyLine);
        cursor += 2;
        continue;
      }
      ...
      if (inNote && lineText.startsWith("*/", cursor)) {
        inNote = false;
        noteStartLine = undefined;
        cursor += 2;
        continue;
      }

      if (!inNote && lineText.startsWith("*/", cursor)) {
        pushDiagnostic(bodyLine, "Unexpected sender note closing marker `*/`.");
        append("source_text", "*/", bodyLine);
        cursor += 2;
        continue;
      }

      append(inNote ? "sender_note" : "source_text", lineText[cursor], bodyLine);
      cursor += 1;
    }
  }

  if (inNote) {
    pushDiagnostic(noteStartLine ?? lines.length, "Unclosed sender note: expected `*/`.");
  }

  for (const segment of segments) {
    if (segment.type === "sender_note") senderNotes.push(segment.text);
  }

  return {
    segments,
    sourceText: segments.filter((segment) => segment.type === "source_text").map((segment) => segment.text).join(""),
    senderNotes,
    diagnostics,
  };
}
...
function parseRouting(
  ...
  const unsupportedGrouped = raw.match(/^\((.+)\)\.(max|synthesis)$/i);
  if (unsupportedGrouped) {
    const method = unsupportedGrouped[2].toLowerCase();
    diagnostics.push({
      level: "error",
      line,
      message:
        method === "max"
          ? "Group routing uses `.best`; `.max` is only valid after a single provider alias."
          : "`.synthesis` is not current ICC syntax. Use `.ensemble`.",
    });
    return undefined;
  }

  const grouped = raw.match(/^\((.+)\)(?:\.(best|ensemble))?$/i);
  if (grouped) {
    ...
    return {
      mode: method ? (method === "best" ? "best" : "synthesis") : "parallel",
      method,
...
function buildChips(
  ...
  if (routing) {
    ...
    if (routing.providers.length > 0 && routing.mode === "synthesis") chips.push(routing.method ?? "ensemble");
