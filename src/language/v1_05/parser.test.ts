...
describe("parseCellDsl", () => {
  ...
  it("parses inline sender notes as body-only non-source segments", () => {
    const parsed = parseCellDsl(
      "> claude.max",
      "Rewrite this paragraph.\n/* Preserve citations and use a calmer tone. */\nDo not change named entities.",
      options,
    );

    expect(parsed.senderNotes).toEqual([" Preserve citations and use a calmer tone. "]);
    expect(parsed.bodySegments?.map((segment) => segment.type)).toEqual(["source_text", "sender_note", "source_text"]);
    expect(parsed.bodySegments?.filter((segment) => segment.type === "source_text").map((segment) => segment.text).join("")).toBe(
      "Rewrite this paragraph.\n\nDo not change named entities.",
    );
    expect(parsed.diagnostics.filter((diagnostic) => diagnostic.level === "error")).toHaveLength(0);
  });

  it("does not parse references inside sender notes as dependencies", () => {
    const parsed = parseCellDsl("> claude.max", "Use this text.\n/* reviewer note: compare with %from c9 */", options);

    expect(parsed.senderNotes?.[0]).toContain("%from c9");
    expect(parsed.references).toHaveLength(0);
    expect(parsed.dependencies).toEqual([]);
    expect(parsed.diagnostics.filter((diagnostic) => diagnostic.level === "error")).toHaveLength(0);
  });

  it("supports multiline sender notes and escaped delimiters", () => {
    const parsed = parseCellDsl(
      "> claude.max",
    ...
    expect(parsed.senderNotes).toEqual([" note line one\nliteral close */ marker "]);
    expect(parsed.bodySegments?.filter((segment) => segment.type === "source_text").map((segment) => segment.text).join("")).toBe(
      "Source /* literal opener.\n\nAfter.",
    );
    expect(parsed.diagnostics.filter((diagnostic) => diagnostic.level === "error")).toHaveLength(0);
  ...
  it("ignores sender-note delimiters inside fenced code blocks", () => {
    const parsed = parseCellDsl(
      "> claude.max",
      "```ts\nconst value = '/* not a note */';\n```\nPlain text.",
      options,
    );

    expect(parsed.senderNotes).toEqual([]);
    expect(parsed.bodySegments?.map((segment) => segment.text).join("")).toBe(
      "```ts\nconst value = '/* not a note */';\n```\nPlain text.",
    );
    expect(parsed.diagnostics.filter((diagnostic) => diagnostic.level === "error")).toHaveLength(0);
  });

  it("diagnoses malformed sender notes", () => {
    const unclosed = parseCellDsl("> claude.max", "Source\n/* missing close", options);
    const unexpected = parseCellDsl("> claude.max", "Source */ stray close", options);
    const nested = parseCellDsl("> claude.max", "/* outer /* inner */", options);
    const header = parseCellDsl("> claude.max /* invalid */", "Source", options);

    expect(unclosed.diagnostics.some((diagnostic) => diagnostic.message.includes("Unclosed sender note"))).toBe(true);
    expect(unexpected.diagnostics.some((diagnostic) => diagnostic.message.includes("Unexpected sender note closing marker"))).toBe(true);
    expect(nested.diagnostics.some((diagnostic) => diagnostic.message.includes("Nested sender notes"))).toBe(true);
    expect(header.diagnostics.some((diagnostic) => diagnostic.message.includes("body-only"))).toBe(true);
  });
